...
## Ratification (2026-07-25) — the thesis is a gate-owned relational invariant, not an absolute win-rate

**Ratified (human):** the thesis is the relational invariant — `diplomat_win% ≥ violent_win% + margin` at stated N — enforced in `make check` (`--sim-flip`), with the diplomat band's lower bound ratified as a thesis-bearing absolute floor (the pair together excludes both inversion AND the least-bad world). Margin **3 points at N=5000**, recorded in the band file. `awe_threshold=1` kept. Option 2 (chase the old absolute 50% by raising goblin receptivity) **rejected**.

**Why this replaced the old "diplomat bot ≥ 50%" north star (the honest history — a caught Goodhart).** The absolute "bot ≥ 50%" was only ever a PROXY for the real thesis, "diplomacy is the clearly-better road." It was a good proxy while the bot's toolkit ≈ the human's; it DRIFTED into a bad one as the game outgrew the instrument — batches 13/14 gave the human cheese→goblin, potion→rat, portal caches, none of which the tactical bots model. Batch 15's correctness fix (goblin awe is talk-gated + struck-blocked — a LEGITIMATE difficulty raise, not a regression) dropped the diplomat bot to 36%/39%, and the response was NOT to silently re-anchor the proxy but to (a) measure, (b) diagnose the proxy drift, (c) route the metric change through human ratification. That is the golem constitution working at the meta level exactly as it works at the object level.

**The conservativeness proof (this is the actual justification for the relation — rider 1).** The relational gate is a sound proxy ONLY because the two bots understate their routes UNEQUALLY, in the right direction. The **violent** bot's kit (fight, rest, USE-potion-when-hurt) is nearly the complete human violent route — its unmodeled remainder (cheese-as-light, portal caches) is small and off-path. The **diplomat** bot's kit excludes cheese→goblin and potion→rat ENTIRELY — whole becalm routes, and the excluded set grows every batch. So the diplomat's number is understated MORE than the violent's ⟹ **bot-relation < human-relation** ⟹ if the gate holds between the bots, it holds a fortiori between the human routes. **Its own expiry condition:** the day a batch adds *violent-route* toolkit the bot doesn't model, this asymmetry must be re-checked — the proxy's conservativeness is not permanent, it's a property of today's instrument.

**The thesis is a PAIR, not the relation alone (rider 2).** A pure relation admits the "least-bad world" the original impasse explicitly rejected — diplomat 12% / violent 6% passes `diplomat > violent + margin` while both routes lose. So the complete encoding is the relational gate **and** an absolute FLOOR on the diplomat: the `tactical-pacifist-band.json` lower bound (currently **30**) is hereby ratified as THESIS-BEARING — load-bearing, not incidental headroom — so no future re-baseline lowers it casually. The two together exclude both inversion (the gate) and the least-bad world (the floor).

**The margin is sized to noise, not vibes (rider 3).** At N=5000 the standard error on a win-rate DIFFERENCE near these values is ~1 point, so today's ~7-point diplomat-over-violent gap is ~7σ real. A margin of 2 would flake on noise; 6 would be brittle against legitimate texture batches that legitimately narrow the gap. **Margin 3 (≈3σ) at N=5000**, both recorded in the band file so the gate's statistical meaning is pinned like every other derived constant (`START_LIGHT`, the dive-cost caches, …).

**Instrument debt, logged honestly.** "The bot no longer models the human toolkit" is a fact about the bot, not a law of nature. An **item-using diplomat bot** (one that picks up and gives cheese/potion) is priced FUTURE WORK; when it lands, the absolute-target question re-opens *against the honest instrument* — so retiring the 50% absolute today is provisional on the instrument, not permanent license. Until then, the relation + the floor are the ratified encoding.

**Why option 2 was rejected (the second reason, beyond over-tuning).** Chasing 50% via goblin `receptivity` is not just over-tuning for a cheese-less bot — `receptivity` is a BLUNTER lever than `awe_threshold`: it moves EVERY talk interaction across all three kinds (rat/goblin/ogre), buying the diplomat's number by reshaping interactions the change was never aimed at. `awe_threshold` is surgical (goblin-awe-completion only); it's the right lever, and 2→1 is the honest gain kept.
